# attribute to Calvin,H (2021), check.py, Project 2, Energy Market and Battery Optimisation,
# MAST30034 Applied Data Science, Sem 2 2021, The University of Melbourne
# modified by Group 14
...
def check_submission(df, include_capacity=False, include_revenue=True):
    initial_charge = df.loc[0, "capacity"]
    battery = Battery(initial_charge)
    flags = []
    capacities = []
    revenues = []
    for idx, (datetime, power, spot_price, capacity) in df.iterrows():
        if include_capacity:
...
if __name__ == "__main__":
    ...
    results = check_submission(submission)
    results.to_csv(args.result[0])
